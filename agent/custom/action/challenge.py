...
class ChallengeAuto(CustomAction):
    ...
    def run(
        ...
        if mode not in mode_allowed:
            mode = "fixed"
            logger.warning(f"warn: 挑战参数错误, 仅允许{mode_allowed}, 已重置为fixed")
        ...
        if mode == "fixed":
            ...
            if index not in index_allowed:
                index = 1
                logger.warning(f"warn: 挑战参数错误, 仅允许{index_allowed}, 已重置为 1")
            logger.info(f"info: 固定选择挑战第 {index + 1} 位")
            context.run_task("ChallengeIndex", pipeline_override={"ChallengeIndex": {"recognition": {"param": {"index": index}}}})

        elif mode == "random":
            index = random.choice([0, 1, 2])
            logger.info(f"info: 随机选择挑战第 {index + 1} 位")
            context.run_task("ChallengeIndex", pipeline_override={"ChallengeIndex": {"recognition": {"param": {"index": index}}}})
            ...
            if reco_detail and reco_detail.hit:
                ...
                index = 1
                logger.info(f"info: 挑战评分识别结果: {ratings}")
                ratings_int = []
                ...
                if all(rating == -1 for rating in ratings_int):
                    index = 1
                    logger.warning("warn: 挑战评分识别失败, 已重置为第2位")
                elif mode == "max":
                    index = ratings_int.index(max(ratings_int))
                    logger.info(f"info: 选择挑战评分最高的第 {index + 1} 位")
                elif mode == "min":
                    index = ratings_int.index(min(ratings_int))
                    logger.info(f"info: 选择挑战评分最低的第 {index + 1} 位")
                else:  # auto
                    reco_detail = context.run_recognition("ChallengeRating", image, pipeline_override={"ChallengeRating": {
                        "recognition": "OCR",
                        "expected": "^\\d{3,6}$",
                    ...
                    if reco_detail and reco_detail.hit:
                        ...
                        try:
                            ...
                            logger.info(f"info: 自动选择挑战第 {index + 1} 位")
                        except ValueError:
                            index = 1
                            logger.warning("warn: 自身挑战评分识别失败, 已重置为第2位")
                context.run_task("ChallengeIndex", pipeline_override={"ChallengeIndex": {"recognition": {"param": {"index": index}}}})
        else:
            logger.error("err: 挑战参数错误, 已退出挑战")
        return True
